...
def get_bonds(atoms: Atoms, covalent_increase_factor: float = 1.25) -> List[List[int]]:
    ...
    # If all atoms were ghost atoms, return empty bonds
    if len(atoms) == 0:
        return []
    ...
    for first_index in range(len(atoms)):
        for second_index in range(first_index + 1, len(atoms)):
            if (
                cr[first_index] + cr[second_index]
            ) * covalent_increase_factor >= distances[first_index, second_index]:
...
def get_coordination_numbers(
    atoms: Atoms, covalent_increase_factor: float = 1.25
) -> List[int]:
    """
    ...
    :rtype: List[int]
    """
    # Use the same ghost-filtering logic as in get_bonds so that only real atoms
    # are included in coordination number calculations
    if "is_ghost" in atoms.arrays:
        is_ghost = atoms.arrays["is_ghost"].astype(bool)
        atoms = atoms[~is_ghost]
    else:
    ...
    # If nothing but ghosts, return empty list
    if len(atoms) == 0:
        return []
...
def get_coordination_numbers_species(
    atoms: Atoms, covalent_increase_factor: float = 1.25
) -> Dict[str, List[int]]:
    """
    ...
    return {
        species: [
            cn for cn, symb in zip(coordination_numbers, symbols) if symb == species
        ]
...
def get_volume_frac(structure: Structure, num_density: bool = False) -> float:
    ...
    if num_density:
        ...
        element_vol = [
            (4 / 3) * np.pi * covalent_radii[elem] ** 3 for elem in element_list
        ]
    return sum(element_vol) / structure.volume


def get_cov(
    gen_atoms: List[ValidAtoms],
    ref_atoms: List[ValidAtoms],
    struc_cutoff: float,
    comp_cutoff: float,
...
def _get_symmetry_dataset_var_prec(
    atoms: Atoms, angle_tolerance: float = -1.0, max_iterations: int = 200
) -> Optional[spglib.SpglibDataset]:
    """
    ...
    while current_group is None or current_group.split()[-1] not in ["(1)", "(2)"]:
        ...
        dataset = spglib.get_symmetry_dataset(
            spglib_cell, symprec=prec, angle_tolerance=angle_tolerance
        )
        prec /= 2.0
        if dataset is None:
            continue
...
def get_space_group(
    ...
    var_prec: bool = False,
) -> Tuple[Optional[str], Optional[int], Optional[str], Optional[Atoms]]:
    """
    ...
    :rtype: Tuple[Optional[str], Optional[int], Optional[str], Optional[Atoms]]
    """
    # Cell arguments for spglib, see
    # https://spglib.readthedocs.io/en/stable/api/python-api/spglib/spglib.html#spglib.get_symmetry.
    spglib_cell = (
    ...
    if var_prec:
        sym_data = _get_symmetry_dataset_var_prec(
            atoms, angle_tolerance=angle_tolerance
        )
    else:
    ...
    if sym_data is None:
        print(
            f"[WARNING] get_space_group: Space group could not be determined ({spglib.get_error_message()})."
        )
    ...
    sym_struc = Atoms(
        numbers=sym_data.std_types,
        scaled_positions=sym_data.std_positions,
        cell=sym_data.std_lattice,
        pbc=True,
    )
...
def _structure_matcher(
    structure_one: Structure,
    structure_two: Structure,
    ltol: float = 0.2,
    stol: float = 0.3,
    angle_tol: float = 5.0,
) -> Optional[float]:
...
def _element_check(atoms_one: Atoms, atoms_two: Atoms, check_reduced: bool) -> bool:
    ...
    if check_reduced:
        ...
        return np.allclose(
            atoms_one_counts / atoms_one_min, atoms_two_counts / atoms_two_min
        )
    else:
...
def _get_match_and_rmsd(
    atoms_one: ValidAtoms,
    atoms_two: ValidAtoms,
    ltol: float,
    stol: float,
    angle_tol: float,
    check_reduced: bool,
...
def _get_match_and_rmsd_sequence(
    atoms_one: ValidAtoms,
    sequence_atoms_two: Sequence[ValidAtoms],
    ltol: float,
    stol: float,
    angle_tol: float,
    check_reduced: bool,
    ...
    for index, atoms_two in enumerate(sequence_atoms_two):
        res = _get_match_and_rmsd(
            atoms_one, atoms_two, ltol, stol, angle_tol, check_reduced
        )
        if res is not None:
...
def match_rmsds(
    atoms_list: Sequence[ValidAtoms],
    ref_list: Sequence[ValidAtoms],
    ltol: float = 0.2,
    stol: float = 0.3,
    angle_tol: float = 5.0,
    ...
    The root-mean-square displacements are normalized by (volume / number_sites) ** (1/3).
    If the two structures do not match, the corresponding root-mean-square displacement is None, and it will be ignored
    in the mean root-mean-square displacement calculation. For the corrected root-mean-square displacement, if a
    structure does not match with any other structure, the corresponding corrected root-mean-square displacement is
    stol.
    ...
    if len(atoms_list) != len(ref_list):
        warnings.warn(
            "The number of structures in the generated atoms list differs from the number of atoms in "
            "the reference list."
        )
    if len(atoms_list) > len(ref_list):
        raise ValueError(
            "The number of structures in the generated atoms list is greater than the number of atoms in "
            "the reference list."
        )
    ...
    if cpu_count > 1:
        res = process_map(
            ...
            atoms_list,
            ref_list,
            desc="Computing matches and RMSDs",
            chunksize=max(min(len(atoms_list) // cpu_count, 100), 1),
    ...
    valid_res = [
        r if atoms.valid and ref_atoms.valid else None
        for r, atoms, ref_atoms in zip(res, atoms_list, ref_list)
    ]

    # counting matches and RMSD
    ...
    # counting valid matches and RMSD
    ...
    # counting corrected RMSD in which non-matched structures are penalized
    ...
    return (
        ...
        float(mean_rmsd),
        valid_match_count / len(atoms_list),
        float(valid_mean_rmsd),
        res,
        valid_res,
        float(corr_rmsd),
...
def metre_rmsds(
    ...
    sequence of validated atoms and return the minimum root-mean-square displacements between the matching structures.
    The validity of structures is determined by the ValidAtoms class.

    The root-mean-square displacements are normalized by (volume / number_sites) ** (1/3).
    If a structure does not match with any other structure, the corresponding root-mean-square displacement is None, and
    it will be ignored in the mean root-mean-square displacement calculation. For the corrected root-mean-square
    displacement, if a structure does not match with any other structure, the corresponding corrected root-mean-square
    displacement is stol.
    ...
    # We cannot use lambda functions so we use (partial) global functions instead.
    match_func = partial(
        _get_match_and_rmsd_sequence,
        sequence_atoms_two=ref_list,
        ltol=ltol,
        stol=stol,
        angle_tol=angle_tol,
        check_reduced=check_reduced,
    ...
    if cpu_count > 1:
        res = process_map(
            match_func,
            atoms_list,
            desc="Computing METRe and RMSDs",
            chunksize=max(min(len(atoms_list) // cpu_count, 100), 1),
            max_workers=cpu_count,
            disable=not enable_progress_bar,
        )
        ...
        if r is not None:
            for rmsd, index in r:  # index is in ref_list
                ...
                if atoms.valid and ref_list[index].valid:
                    if valid_rmsds_ref[index] is None:
                        valid_rmsds_ref[index] = rmsd
                    elif rmsd < valid_rmsds_ref[index]:
                        valid_rmsds_ref[index] = rmsd

    # counting matches and RMSD
    match_count = sum(rmsd is not None for rmsd in rmsds_ref)
    filtered_rmsds = [rmsd for rmsd in rmsds_ref if rmsd is not None]
    mean_rmsd = np.mean(filtered_rmsds)

    # counting valid matches and RMSD
    valid_match_count = sum(rmsd is not None for rmsd in valid_rmsds_ref)
    filtered_valid_rmsds = [rmsd for rmsd in valid_rmsds_ref if rmsd is not None]
    valid_mean_rmsd = np.mean(filtered_valid_rmsds)

    # counting corrected RMSD in which non-matched structures are penalized
    corr_rmsds = [rmsd if rmsd is not None else stol for rmsd in rmsds_ref]
    corr_valid_rmsds = [rmsd if rmsd is not None else stol for rmsd in valid_rmsds_ref]
    corr_rmsd = np.mean(corr_rmsds)
    valid_corr_rmsd = np.mean(corr_valid_rmsds)

    return (
        match_count / len(ref_list),
        float(mean_rmsd),
        valid_match_count / len(ref_list),
        float(valid_mean_rmsd),
        rmsds_ref,
        valid_rmsds_ref,
        float(corr_rmsd),
