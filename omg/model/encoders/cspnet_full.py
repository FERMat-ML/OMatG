...
class CSPNetFull(Encoder, CSPNet):
    def __init__(
        ...
        self.node_embedding = nn.Embedding(
            max_atoms + 20, hidden_dim
        )  # Supports ghost atoms up to species 119
    ...
    def _forward(
        self, atom_types, frac_coords, lattices, num_atoms, node2graph, t, prop=None
    ):
        ...
        if prop is not None:
            prop_indicator = prop_indicator(
                batch_size=len(num_atoms), p_uncond=0.2
            )  # p_uncond should be in yaml

        for i in range(0, self.num_layers):
            if prop is not None:
                node_features = self.adapters[i](
                    node_features, prop, prop_indicator, num_atoms
                )
