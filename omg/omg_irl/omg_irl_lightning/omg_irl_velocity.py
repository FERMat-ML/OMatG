...
class OMGIRLVelocity(OMGIRLScale):
    """
    Velocity-based Open Materials Generation with Inference-time Reinforcement Learning (OMatG-IRL) using
    group-relative policy optimization (GRPO) and proximal policy optimization (PPO) within the PyTorch Lightning
    framework.

    This class reinforces a velocity field of a base OMatG model.

    Exploration is enabled by adding Gaussian noise with time-dependent standard deviation sigma(t) to the velocity
    field predicted by the base model, resulting in the following Euler-Maruyama update for the variable x_t:
    x_t+dt = x_t + b(x_t, t) * dt + sigma(t) * randn * sqrt(dt),
    where randn is a standard normal random variable with the same dimension as b(x_t, t), and sigma(t) is a
    (potentially learnable) noise schedule.
    ...
    def __init__(self, reward: Reward, reference_noise_schedules: dict[str, NoiseSchedule],
        ...
        super().__init__(reward=reward, reference_noise_schedules=reference_noise_schedules,
                         noise_schedules=noise_schedules, relative_costs=relative_costs, scale_model=None,
        ...
        for t_index in trange(1, len(times), desc="Integrating with reinforced model", position=1, leave=False,
            ...
            if self.integrate_pos:
                if DataField.pos in self.disable_fields:
                    ...
                    velocity = base_model_output[DataField.pos.name + "_b"]
            ...
            if self.integrate_cell:
                if DataField.cell in self.disable_fields:
                    # Take a standard Euler step (i.e., Euler-Maruyama without noise).
                    assert base_model_output is not None
                    velocity = base_model_output[DataField.cell.name + "_b"]
                    noise = torch.zeros_like(velocity)
                else:
                    sigma = self.noise_schedules[DataField.cell].noise(t)  # Scalar.
                    assert sigma.ndim == 0
