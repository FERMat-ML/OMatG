...
class CRMSEReward(Reward):
    """
    Reward function that reduces a quantity similar to the corrected root-mean-square error (cRMSE) between generated
    structures and known stable structures in a reference dataset.

    During the reward computation, each generated structure is compared to all structures in the reference dataset with
    the same reduced composition using Pymatgen's StructureMatcher. The cRMSE-like quantity is calculated as the minimum
    RMSD among all matched structures. If no match is found within the specified tolerances, a penalty equal to the
    structure tolerance (stol) is assigned. The RMSD is computed using Pymatgen's get_rms_dist method and is normalized
    by (Vol / nsites) ** (1/3).  Hence, the cRMSE-like quantity is similarly normalized.

    This reward effectively inverts the standard cRMSE computation, which identifies best matches from the generated set
    for each reference structure.

    The training, validation, and prediction dataset structures are indexed by their reduced composition for efficient
    lookup. The indices are built once when set_datasets is called.
    ...
    :raises ValueError:
        ...
        If number_cpus is not None and less than 1.
    """
    ...
    def set_train_dataset(self, train_dataset: OMGDataset) -> None:
        ...
        This method should be called before compute is called. It builds a dictionary that maps reduced compositions to
        lists of Pymatgen Structures with that composition in the training dataset for efficient lookup during reward
        computation.
    ...
    def set_val_dataset(self, val_dataset: OMGDataset) -> None:
        ...
        This method should be called before compute is called. It builds a dictionary that maps reduced compositions to
        lists of Pymatgen Structures with that composition in the validation dataset for efficient lookup during reward
        computation.
    ...
    def set_pred_dataset(self, pred_dataset: OMGDataset) -> None:
        ...
        This method should be called before compute is called. It builds a dictionary that maps reduced compositions to
        lists of Pymatgen Structures with that composition in the prediction dataset for efficient lookup during reward
        computation.
    ...
    def compute(self, structures: Sequence[Structure], stage: ComputeStage,
        ...
        :raises RuntimeError:
            If the relevant dataset for the given stage has not been set.
        """
        if stage == ComputeStage.TRAIN:
            ...
            assert stage == ComputeStage.PRED
