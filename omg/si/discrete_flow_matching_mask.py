...
class DiscreteFlowMatchingMask(StochasticInterpolantSpecies):
    ...
    def integrate(self, model_function: Callable[[torch.Tensor, torch.Tensor], tuple[torch.Tensor, torch.Tensor]],
        ...
        # Shape (sum(n_atoms),).
        will_unmask = torch.rand(x_t, device=x_t.device) < (time_step * (1.0 + self._noise * time) / (1.0 - time))
        ...
        will_mask = torch.rand(x_t, device=x_t.device) < time_step * self._noise  # Shape (sum(n_atoms),).
