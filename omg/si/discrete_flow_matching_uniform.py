...
class DiscreteFlowMatchingUniform(StochasticInterpolant):
    ...
    def integrate(self, model_function: Callable[[torch.Tensor, torch.Tensor], tuple[torch.Tensor, torch.Tensor]],
        ...
        for _ in range(self._number_integration_steps):
            ...
            shifted_x_1_hot = functional.one_hot(shifted_x_1, num_classes=MAX_ATOM_NUM - 1)
            dpt = shifted_x_1_hot - (1.0 / (MAX_ATOM_NUM - 1))  # Shape (sum(n_atoms), MAX_ATOM_NUM - 1).
            ...
            # Shape (sum(n_atoms), MAX_ATOM_NUM - 1).
            pt = (t * shifted_x_1_hot) + (1.0 - t) * (1.0 / (MAX_ATOM_NUM - 1))
            ...
            if self._noise > 0.0:
                ...
                rate_db[shifted_x_1 != shifted_x_t] = (((MAX_ATOM_NUM - 1) * t) + 1.0 - t) / (1.0 - t + eps)
                rate_db *= self._noise
            rate += rate_db
