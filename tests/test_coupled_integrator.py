...
def test_coupled_integrator():
    ...
    if isinstance(ode_interp._interpolant, PeriodicScoreBasedDiffusionModelInterpolant):
        pbc_flag = True
    elif isinstance(ode_interp._interpolant, PeriodicLinearInterpolant):
        pbc_flag = True
    elif isinstance(sde_interp._interpolant, PeriodicScoreBasedDiffusionModelInterpolant):
        pbc_flag = True
    elif isinstance(sde_interp._interpolant, PeriodicLinearInterpolant):
        pbc_flag = True
    else:
        pbc_flag = False
    ...
    for i in range(0, len(times)):
        ...
        # Check approximation
        if pbc_flag:
            diff = torch.abs(pos_true - inter[i].pos)
            pos_true_prime = torch.where(diff >= 0.5, pos_true + torch.sign(inter[i].pos - 0.5), pos_true)
